...
public class Escape implements IUserCommandHandler
{
	...
	public void useUserCommand(int id, Player player)
	{
		...
		if (player.isGM())
			player.getAI().tryTo(IntentionType.CAST, new SkillUseHolder(player, player, SkillTable.getInstance().getInfo(2100, 1), false, false), null);
		else
		{
			player.sendPacket(new PlaySound("systemmsg_e.809"));
			player.sendPacket(SystemMessageId.STUCK_TRANSPORT_IN_FIVE_MINUTES);
			
			player.getAI().tryTo(IntentionType.CAST, new SkillUseHolder(player, player, SkillTable.getInstance().getInfo(2099, 1), false, false), null);
		}
	}
