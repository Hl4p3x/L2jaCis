...
public class Tutorial extends Quest
{
	...
	public String onTimer(String name, Npc npc, Player player)
	{
		if (name.startsWith("QT"))
		{
			...
			if (Ex == -2)
			{
				...
				if (!st.hasQuestItems(TUTORIAL_GUIDE))
					st.giveItems(TUTORIAL_GUIDE, 1);
				
				st.set("Ex", "-3");
				st.playTutorialVoice(evt._initialVoice);
				
				cancelQuestTimers("QT");
				startQuestTimer("QT", null, player, 30000);
				
				st.showTutorialHTML(evt._initialHtm);
			}
			else if (Ex == -3)
			{
				st.set("Ex", "0");
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			if (player.getStatus().getLevel() < 6 && st.getInt("onlyone") == 0)
			{
				switch (st.getInt("ucMemo"))
				{
					...
					case 2:
						final QuestState qs101 = player.getQuestState(qn101);
						final QuestState qs102 = player.getQuestState(qn102);
						final QuestState qs103 = player.getQuestState(qn103);
						final QuestState qs104 = player.getQuestState(qn104);
						final QuestState qs105 = player.getQuestState(qn105);
						final QuestState qs106 = player.getQuestState(qn106);
						final QuestState qs107 = player.getQuestState(qn107);
						final QuestState qs108 = player.getQuestState(qn108);
					...
					case 3:
						...
						else if (st.hasQuestItems(BLUE_GEMSTONE))
			...
			switch (Integer.valueOf(event.substring(2)))
			{
					...
					switch (level)
					{
						case 5:
							if (qLvl < 5)
							{
								...
								st.playSound(QuestState.SOUND_TUTORIAL);
								st.playTutorialVoice((player.isMageClass()) ? "tutorial_voice_015" : "tutorial_voice_014");
							}
							break;
						...
						case 19:
							if (qLvl < 19)
							{
								...
								st.playSound(QuestState.SOUND_TUTORIAL);
								st.playTutorialVoice("tutorial_voice_022");
							}
							break;
				...
				case 45:
					if (level < 6 && st.getInt("HP") == 0)
					{
						...
						st.onTutorialClientEvent(8388608);
						st.playSound(QuestState.SOUND_TUTORIAL);
