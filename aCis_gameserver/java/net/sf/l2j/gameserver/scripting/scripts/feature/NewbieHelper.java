...
public class NewbieHelper extends Quest
{
	...
	public NewbieHelper()
	{
		...
		addStartNpc(30009, 30019, 30131, 30400, 30530, 30575, 30008, 30017, 30129, 30370, 30528, 30573, 30598, 30599, 30600, 30601, 30602, 31076, 31077);
	...
	public String onTimer(String name, Npc npc, Player player)
	{
		...
		else if (name.equalsIgnoreCase("TimerEx_GrandMaster"))
		{
			if (qs.getInt("Ex") >= 4)
			{
				...
				st.playSound(QuestState.SOUND_TUTORIAL);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		{
			st.addRadar(NEWBIE_GUIDE_LOCS.get(npc.getNpcId()));
			
			final int itemId = getItemId(npc.getNpcId());
			if (st.hasQuestItems(itemId) && st.getInt("onlyone") == 0)
			{
				...
				if (player.isMageClass() && player.getClassId() != ClassId.ORC_MYSTIC)
				{
					...
					st.giveItems(SPIRITSHOT_NO_GRADE_FOR_BEGINNERS, 100);
				}
				else
				{
					st.playTutorialVoice("tutorial_voice_026");
					st.giveItems(SOULSHOT_NO_GRADE_FOR_BEGINNERS, 200);
				}
		...
		else if (event.startsWith("SupportMagic"))
		{
			...
			final boolean isAlikeMage = player.isMageClass() && player.getClassId() != ClassId.ORC_MYSTIC && player.getClassId() != ClassId.ORC_SHAMAN;
			
			final int playerLevel = player.getStatus().getLevel();
			
			// If the player is too low level, display a message and return.
			if (playerLevel < NewbieBuffData.getInstance().getLowestBuffLevel(isAlikeMage))
			...
			else
			{
				...
				int i = 0;
				for (NewbieBuffHolder buff : NewbieBuffData.getInstance().getValidBuffs(isAlikeMage, playerLevel))
					ThreadPool.schedule(() -> st.callSkill(buff.getSkill()), 1000 * i++);
		...
		else if (event.startsWith("GiveBlessing"))
		{
			...
			{
				ThreadPool.schedule(() -> st.callSkill(FrequentSkill.BLESSING_OF_PROTECTION.getSkill()), 1000);
				return null;
			}
		}
	...
	public String onFirstTalk(Npc npc, Player player)
	{
		...
		if (npcId >= 30598 && npcId <= 30602)
		{
			if (!st.isCompleted())
			{
				...
				st.giveItems(NEWBIE_TRAVEL_TOKEN, 12);
		...
		{
			final int level = player.getStatus().getLevel();
			if (level >= 10 || st.getInt("onlyone") == 1)
			...
			else if (step == 1 && qs.getInt("Ex") <= 2)
			{
				if (st.hasAtLeastOneQuestItem(BLUE_GEMSTONE))
				{
					qs.set("Ex", "3");
					qs.set("ucMemo", "3");
					
					st.set("step", "2");
					st.takeItems(BLUE_GEMSTONE, -1);
					st.giveItems(getItemId(npcId), 1);
	...
	public String onKill(Npc npc, Creature killer)
	{
		...
		if (ex <= 2 && qs.getInt("Gemstone") == 0 && Rnd.get(100) < 25)
		{
